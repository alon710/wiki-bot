...
class WikipediaService:
    ...
    def get_random_hebrew_article(self, max_retries: int = 5) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.error("Failed to fetch suitable random Hebrew article after retries",
    ...
    def _fetch_random_article_with_retry(self) -> Dict[str, Any]:
        ...
        try:
            # Get random page
            random_page = self.client.page("Special:Random")
            
            # If we get a redirect, follow it
            if random_page.exists():
                actual_page = self.client.page(random_page.title)
                ...
                if self._is_suitable_hebrew_article(actual_page):
                    ...
                    logger.info("Random Hebrew article fetched successfully",
                               title=actual_page.title)
                ...
                logger.debug("Page does not exist, retrying")
                ...
                if options and len(options) > 0:
                    try:
                        ...
                        if self._is_suitable_hebrew_article(disambig_page):
                            ...
                            logger.info("Hebrew disambiguation article fetched successfully",
                                       title=disambig_page.title)
                            
                            return article_data
                        else:
                            logger.debug("Hebrew disambiguation article not suitable, retrying",
                                       title=disambig_page.title)
                        ...
                        logger.warning("Error handling Hebrew disambiguation page",
            ...
            logger.warning("Error fetching random Hebrew article, will retry",
                         error=str(e))
    ...
    def _is_suitable_hebrew_article(self, page: wikipediaapi.WikipediaPage) -> bool:
        ...
        logger.debug("Hebrew article validation passed",
                    title=page.title,
    ...
    def get_hebrew_page_by_title(self, title: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Error fetching Hebrew page by title",
                        title=title,
                        error=str(e))
            return None
